...
istall.packages(c("tidyverse", "dslabs"))
library( tidyverse )
installed.packages ()
...
c <- -1
(- b - sqrt ( b ^ 2 - 4 * a * c ))/(2 * a )
## Создание и сохранение скрипта значительно упрощает выполнение кода
...
heights <- c(56,56,78,99,76,99,08)
heights * 2.54
heights -70
...
my_df <- data.frame(name = states, rank1 = ranks)
my_df
...
filter(murders, rate>4.4) %>% top_n(5)


# Use filter to create a new data frame no_south
no_south <- filter(murders, region != "South")
no_south
# Usno_southe nrow() to calculate the number of rows
nrow(no_south)
